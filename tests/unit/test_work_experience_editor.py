...
from conftest import TEST_CANDIDATE_ID


# ─── Tests ────────────────────────────────────────────────────


async def test_get_work_experience_page_contains_profile_json(
    test_env, test_candidate_source_data
):
    ...
    async with httpx.AsyncClient(
        ...
        assert test_candidate_source_data["summary"][:40] in resp.text


async def test_put_saves_and_persists_edits(test_env, test_candidate_source_data):
...
async def test_put_rejects_invalid_payload(test_env, test_candidate_source_data):
...
async def test_put_returns_404_for_unknown_candidate(
    test_env, test_candidate_source_data
):
...
async def test_save_profile_updates_db_and_cache(
    test_env, test_candidate_source_data
):
    ...
    assert (
        get_profile(TEST_CANDIDATE_ID).summary == "new summary via save_profile"
    )
    assert "new summary via save_profile" in get_profile_json(TEST_CANDIDATE_ID)
...
async def test_put_add_and_remove_work_entry(
    test_env, test_candidate_source_data
):
    ...
    async with httpx.AsyncClient(
        ...
        resp2 = await client.put(
            f"/api/candidates/{TEST_CANDIDATE_ID}/work-experience", json=d2
        )
...
async def test_put_add_and_remove_education_subject(
    test_env, test_candidate_source_data
):
...
async def test_put_add_and_remove_publication_author(
    test_env, test_candidate_source_data
):
